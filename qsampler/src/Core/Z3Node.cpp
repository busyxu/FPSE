...
// ********* TAKEN FROM Z3 SOURCES ********* //

static bool is_smt2_simple_symbol_char(char s) {
    return 
        ('0' <= s && s <= '9') ||
        ('a' <= s && s <= 'z') ||
        ('A' <= s && s <= 'Z') ||
...
static bool is_smt2_quoted_symbol(char const * s) {
    if (s == 0)
        return false;
    if ('0' <= s[0] && s[0] <= '9')
        return true;
...
std::string Z3FuncDeclHandle::getName(bool smtlibEscape) const {
  ...
  auto res = std::string(::Z3_get_symbol_string(context, sym));

  if (!smtlibEscape){
    return res;
  } else{
    if (is_smt2_quoted_symbol(res.c_str())){
      return "|" + res + "|";
    } else{
      return res;
    }
...
void
Z3ApplyResultHandle::convertModelForGoalInPlace(unsigned index,
                                         Z3ModelHandle toConvert) {
  assert(index < getNumGoals());
  assert(!toConvert.isNull());
  assert(toConvert.getContext() == context);
  Z3_apply_result_convert_model_in_place(context, node, index, toConvert);
}
}
}

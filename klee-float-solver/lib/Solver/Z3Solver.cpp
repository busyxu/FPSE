...
bool Z3SolverImpl::internalRunSolver(
  ...
  if (runStatusCode == SolverImpl::SOLVER_RUN_STATUS_SUCCESS_SOLVABLE ||
      runStatusCode == SolverImpl::SOLVER_RUN_STATUS_SUCCESS_UNSOLVABLE) { // unsolvale expression solving success.
    if (hasSolution) { //reverse into solver. if hassolution means, the cons is unsat, so is invaild
      ++stats::queriesInvalid;
    } else {// reverse into solver. if not hassolution means, the cons always sat. so is valid
      ++stats::queriesValid;
    }
    return true; // success
  }

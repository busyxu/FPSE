...
//      solver.setOption("output-language", "smtlib"); // output-language
    solver.setOption("tlimit-per", "30000");//30s == 30000ms
//        solver.setOption("tlimit", "30000");//30s == 30000ms
    solver.setLogic("ALL");
...
Term CVC5Solver::bvMinusOne(unsigned width) {
...
Term CVC5Solver::bvConst64(unsigned width, uint64_t value) {
  Term temp = solver.mkBitVector(width,value);
  return temp;
}
  ...
  if(sort.isFloatingPoint()){
    ...
    switch (bitWidth) {
      ...
      case Expr::Int128:{
        ...
        uint32_t expWidth = getFloatExpFromBitWidth(bitWidth);
        uint32_t sigWidth = getFloatSigFromBitWidth(bitWidth);
        Op bv2fp = solver.mkOp(FLOATINGPOINT_TO_FP_FROM_SBV,{expWidth, sigWidth});
        Term tofp = solver.mkTerm(bv2fp,{roundMode, e});
        return tofp;
...
Term CVC5Solver::castToBitVector(Term  e) {
  ...
  if (currentSort.isBitVector())
    ...
    switch (floatWidth) {
      ...
      case Expr::Int64:
      case Expr::Int128: {
//        Term temp = castToFloat(e);
        Op tosbv = solver.mkOp(FLOATINGPOINT_TO_SBV, {floatWidth});
        Term res = solver.mkTerm(tosbv, {roundMode,e});
        return res;
    }
      default:
    ...
    case Expr::FPToUI: {
      ...
      *width_out = ce->getWidth();
      ...
      if (*width_out == srcWidth){
        Op fp2ubv = solver.mkOp(FLOATINGPOINT_TO_UBV,{(uint32_t)(*width_out)});
        ...
        Op fp2ubv = solver.mkOp(FLOATINGPOINT_TO_UBV,{(uint32_t)(*width_out)});
        return solver.mkTerm(fp2ubv,{roundMode,targetFP});
      }
...
//      Op fp2sbv = solver.mkOp(FLOATINGPOINT_TO_SBV,{(uint32_t)(*width_out)});
//      return solver.mkTerm(fp2sbv,{src});
...
//        return castToFloat(src);
        uint32_t expWidth = getFloatExpFromBitWidth(*width_out);
        uint32_t sigWidth = getFloatSigFromBitWidth(*width_out);
        Op ubvTofp = solver.mkOp(FLOATINGPOINT_TO_FP_FROM_UBV,{expWidth, sigWidth});
        return solver.mkTerm(ubvTofp,{roundMode, src});
      }else {
        return src;
      }
    }
...
//        return castToFloat(src);
        uint32_t expWidth = getFloatExpFromBitWidth(*width_out);
        uint32_t sigWidth = getFloatSigFromBitWidth(*width_out);
        Op sbvTofp = solver.mkOp(FLOATINGPOINT_TO_FP_FROM_SBV,{expWidth, sigWidth});
        return solver.mkTerm(sbvTofp,{roundMode, src});
      }else {
        return src;
      }
    }
...
//  return solver.mkVar(getBvSort(bitWidth),{prefix});
  return solver.mkConst(getBvSort(bitWidth), {prefix});
}
...
//          ref<FOEqExpr> leftEq = FOEqExpr::create(leftExtAbs,DZeroExtExpr);
//          ref<FOEqExpr> rightEq = FOEqExpr::create(rightExtAbs,DZeroExtExpr);
      ref<Expr> limit = FOLtExpr::create(leftExtAbs,DZeroExtExpr);
      return construct(limit,width_out);
    }
    if (e->getKind() == Expr::FInvalidLogCheck){
//          ref<FOEqExpr> leftEq = FOEqExpr::create(leftExtAbs,DZeroExtExpr);
//          ref<FOEqExpr> rightEq = FOEqExpr::create(rightExtAbs,DZeroExtExpr);
      ref<Expr> limit = FOLeExpr::create(leftExtAbs,DZeroExtExpr);
      return construct(limit,width_out);
    }
    if (e->getKind() == Expr::FInvalidPowCheck){
//          ref<FOEqExpr> leftEq = FOEqExpr::create(leftExtAbs,DZeroExtExpr);
//          ref<FOEqExpr> rightEq = FOEqExpr::create(rightExtAbs,DZeroExtExpr);
      ref<Expr> limit = FOEqExpr::create(leftExtAbs,DZeroExtExpr);
      return construct(limit,width_out);
    }
    else{
      assert(false && "unsupport fpcheck expr !");
...
int CVC5Solver::invokeCVC5Solver(
        ConstraintSet &constraints,
        const std::vector<const Array *> *objects,
        std::vector<std::vector<unsigned char> > *values) {

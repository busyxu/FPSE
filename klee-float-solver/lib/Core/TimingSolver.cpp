...
bool TimingSolver::evaluate(ExecutionState &state, ref<Expr> expr,
                            ...
                            bool useSeed) {// .h assignment true
  ...
  if (useSeed){
    ...
    ref<Expr> tmp_expr = expr;

//    ref<Expr> expr_1 = state.assignSeed.evaluate(tmp_expr); //[dbg yx]
//      expr = state.assignzeros.evaluate(tmp_expr); //[dbg yx]

      expr = ConstantExpr::alloc(0, Expr::Bool);
//      expr_1->dump();
//      expr->dump();
//
    if(state.samplingSeedList.size()>0){
        for(int i=0; i < state.samplingSeedList.size(); i++){
            expr = state.samplingSeedList[i].evaluate(tmp_expr);
            if (dyn_cast<ConstantExpr>(expr)->isTrue()) {
//                tmp_expr->dump();
//                 llvm::errs()<<"success hit!!!"<<"\n";
                break;
            }
        }
    }

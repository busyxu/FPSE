...
// add by yx
cl::opt<std::string> samplingAlg(
        "sampling-alg",
        cl::init("moea_sampling"),
        cl::desc("sampling algorithm."),
        cl::cat(ExtCallsCat));
...
//  BOOLECTOR,
  Z3,     // Only use Z3
  JFS,     // Only use JFS
  DREAL_JFS,   // Only use DReal with JFS
  DREAL,    // DReal with internal search, SOTA work
  Z3_DREAL_JFS,   // Use SMT to check UNSAT, then use DReal and JFS
  Z3_JFS,
  GOSAT,
  Z3_GOSAT,
  Z3_OPTSAT,   // OPTSAT with Z3
  Z3_QSampler,
  OPTSAT,
  BITWUZLA,
  MATHSAT5,
...
cl::opt<SolverType> SolverTypeDecision(
    ...
    cl::desc("Specify the external call policy"),
    cl::values(
        clEnumValN(SolverType::Z3, "z3",
                   "Always use Z3 to solve all function.(default)"),
        clEnumValN(SolverType::JFS, "jfs",
                   "Always use Fuzzing by JFS to solve all function."),
        clEnumValN(SolverType::DREAL_JFS, "dreal-jfs",
                   "Always use DReal to solve all function."),
        clEnumValN(SolverType::DREAL, "colibri",
                   "POPL13, DReal with internal search, SOTA work."),
        clEnumValN(SolverType::Z3_DREAL_JFS, "z3-dreal-jfs",
                   "APSEC22 work implementation."),
        clEnumValN(SolverType::Z3_JFS, "z3-jfs",
                   "Synergy work implementation."),
        clEnumValN(SolverType::Z3_GOSAT, "gosat",
                   "GoSat solver."),
        clEnumValN(SolverType::Z3_OPTSAT, "qsf",
                   "OptSat with Z3 solver."),
        clEnumValN(SolverType::Z3_QSampler, "qsampler",
                   "QSampler with Z3 solver."),
        clEnumValN(SolverType::OPTSAT, "optsat",
                   "OptSat solver."),
...
//        llvm::errs() << "[add cond (True)]: " << condition << "\n";
      ...
      otherState->addInitialConstraint(Expr::createIsZero(condition));
        otherState->samplingSeedList = current.samplingSeedList;
      addedStates.push_back(otherState);
      processTree->attach(current.ptreeNode, otherState,&current, reason);// [dbg yxxxx]
...
//      llvm::errs() << "[add cond (False)]: " << condition << "\n";
      ExecutionState *otherState = current.copyConcrete();
      current.addInitialConstraint(Expr::createIsZero(condition));
...
void Executor::executeCall(ExecutionState &state, KInstruction *ki, Function *f,
  ...
  if (SolverTypeDecision == SolverType::DREAL_JFS ||
      SolverTypeDecision == SolverType::DREAL ||
      SolverTypeDecision == SolverType::JFS ||
      SolverTypeDecision == SolverType::Z3_DREAL_JFS ||
      SolverTypeDecision == SolverType::Z3_JFS ||
      SolverTypeDecision == SolverType::GOSAT ||
      SolverTypeDecision == SolverType::OPTSAT ||
      SolverTypeDecision == SolverType::Z3_OPTSAT ||
      SolverTypeDecision == SolverType::Z3_QSampler ||
...
//          SolverTypeDecision == SolverType::CVC5 ||
  ...
  else if(SolverTypeDecision == SolverType::Z3 ||
...
void Executor::getConcreteAssignSeedZ3(ExecutionState &state, bool &checkValid){
...
//          std::cerr << v << ",  " << solution[v].mid() << "\n";
      ...
      if (res == 0) {
        ...
        getConcreteAssignSeedJFS(state,checkValid,1);//no fix
        return;;
      }
    }else{
      /*if (!unSupport){
...
void Executor::getConcreteAssignSeedZ3DRealJFS(ExecutionState &state, bool &checkValid, std::string filename){
...
//      }
      ...
      if (res == 0) {
        ...
        getConcreteAssignSeedJFS(state,checkValid, 3);
        return;
      }
    }
  ...
  getConcreteAssignSeedJFS(state,checkValid, 1.5);//smt is 1/3 so this is 2/3
}

// use z3Assign to simplify to constraints for drealCons
void Executor::getConcreteAssignSeedZ3JFS(ExecutionState &state, bool &checkValid){
...
//      }
    ...
    if(intosmtFlag)
      getConcreteAssignSeedJFS(state,checkValid, 2);
    else
      getConcreteAssignSeedJFS(state,checkValid, 1);
  }
  return;
}

void Executor::getConcreteAssignSeedZ3DReal(ExecutionState &state, bool &checkValid){
...
//      llvm::errs()<<"=================DReal: Z3 Result: SAT=================\n";
      ...
      if (!state.fakeState){
        klee_warning("CVC5Real: Z3 solving UNKNOWN and evaluate SUCCESS and remove the state !");
        removedStates.push_back(&state);
        concreteHalt = true;
      }
      checkValid = false;
...
void Executor::getConcreteAssignSeedJFS(ExecutionState &state, bool &checkValid, int split_t){
...
void Executor::getConcreteAssignSeedZ3GoSAT(ExecutionState &state, bool &checkValid){
//  llvm::errs()<<"===========call gosat solver===========\n";
    std::vector< std::vector<unsigned char> > z3Values,gosatValues,zeroValue;
    ...
    if (!z3Objects.empty() && !z3Cons.empty()){//z3 objective noempty; have gosat unsupport cons
        ...
        if (success){
            ...
            if (!useGosatFlag){//no gosat constaints, don't need gosat solving.
                klee_warning("Z3GoSAT: Z3 evaluate SUCCESS !");
                state.assignSeed = currentAssign;
                checkValid = true;
                return ;
            }
            ...
            if (!state.fakeState){
                klee_warning("Z3GoSAT: Z3 solving UNSAT and remove the state !");
                removedStates.push_back(&state);
                concreteHalt = true;
            }
            checkValid = false;
...
//    bool success = invokeGoSAT(smtStr,varTypes, gosatObjects,gosatValues);
        auto start = std::chrono::high_resolution_clock::now();
        bool success = invokeGoSAT(smtLibStr.c_str(),varTypes, gosatObjects,gosatValues,zeroValue,"gosat");
        auto end = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> duration = end - start;
        double milliseconds = duration.count() * 1000.0;
        llvm::errs() << ">>>gosat exec time: " << milliseconds << " ms\n";
...
//      currentAssign.dump();
            ...
            if (res == 0){
                klee_warning("Z3GoSAT: GoSAT solving SAT and evaluate SUCCESS !");
                state.assignSeed = currentAssign;
                Assignment assignZeros = Assignment(gosatObjects,gosatValues,true);
                state.assignzeros = assignZeros;
                checkValid = true;
                return ;
            }
            // solve failed, remove this fuzzing failed state
            klee_warning("Z3GoSAT: GoSAT solving SAT and evaluate FAILURE and remove the state !");
        }
        else
            klee_warning("Z3GoSAT: GoSAT solving UNKNOWN and evaluate FAILURE and remove the state !");
    }
...
void Executor::getConcreteAssignSeedZ3OptSAT(ExecutionState &state, bool &checkValid){
//  llvm::errs()<<"===========call gosat solver===========\n";
  std::vector< std::vector<unsigned char> > z3Values,gosatValues,zeroValues;
  std::vector< const Array*> z3Objects, gosatObjects;
  std::map<std::string,std::string> varTypes;
  std::set<std::string> z3VarName, gosatVarName;
  ConstraintSet constraints(state.constraints),z3Cons, gosatCons;
...
//  llvm::errs()<<"===========\n";
  ...
  if (!z3Objects.empty() && !z3Cons.empty()){//z3 objective noempty; have gosat unsupport cons
    ...
    if (success){
      ...
      // easy constaints solution luckcy satisfied hard constaints
      if (!useGosatFlag){//no gosat constaints, don't need gosat solving.
        klee_warning("Z3OPTSAT: Z3 evaluate SUCCESS !");
        state.assignSeed = currentAssign;
        checkValid = true;
        return ;
      }
      ...
      if (!state.fakeState){
        klee_warning("Z3OPTSAT: Z3 solving UNSAT and remove the state !");
        removedStates.push_back(&state);
        concreteHalt = true;
      }
      checkValid = false;
...
//    bool success = invokeGoSAT(smtStr,varTypes, gosatObjects,gosatValues);
    auto start = std::chrono::high_resolution_clock::now();
    bool success = invokeGoSAT(smtLibStr.c_str(),varTypes, gosatObjects,gosatValues, zeroValues,"optsat");
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> duration = end - start;
    double milliseconds = duration.count() * 1000.0;
    llvm::errs() << ">>>optsat exec time: " << milliseconds << " ms\n";
...
//      currentAssign.dump();
      int res = checkAssignmentValid(currentAssign, constraints);//检查gosat求出来的解是否 是有效解
      if (res == 0){
        klee_warning("Z3OPTSAT: OPTSAT solving SAT and evaluate SUCCESS !");
        state.assignSeed = currentAssign;
        Assignment assignZero = Assignment(gosatObjects,zeroValues,true);
//        assignZero.dump();
        state.assignzeros = assignZero;
        checkValid = true;
        return ;
      }
      // solve failed, remove this fuzzing failed state
      klee_warning("Z3OPTSAT: OPTSAT solving SAT and evaluate FAILURE and remove the state !");
    }
    else
      klee_warning("Z3OPTSAT: OPTSAT solving UNKNOWN and evaluate FAILURE and remove the state !");
  }
...
//
//void Executor::getConcreteAssignSeedZ3QSampler(ExecutionState &state, bool &checkValid){
////  llvm::errs()<<"===========call gosat solver===========\n";
//    std::vector< std::vector<unsigned char> > z3Values;
...
//    }
//
//    if (state.samplingSeedList.size()>0){
//        bool flag = false;
////        llvm::errs()<<"sampling_size: "<<state.samplingSeedList.size()<<"\n";
//        for(int i=0; i<state.samplingSeedList.size(); i++) {
//            Assignment currentAssign = state.assignSeed;
//            currentAssign.updateValues(state.samplingSeedList[i]);
//            int res = checkAssignmentValid(currentAssign,constraints);//检查gosat求出来的解是否 是有效解
//            if (res == 0){
//                state.assignSeed = currentAssign;
//                flag = true;
...
//        }
//        if(flag){
//            klee_warning("Z3QSampler: Sampling evaluate SUCCESS !");
//            checkValid = true;
//            return ;
//        }
//        klee_warning("Z3QSampler: Sampling evaluate FAILURE !");
//    }
//
//    bool unSupport = false;
//    for (const auto &cons : constraints){
...
//                klee_warning("Z3OPTSAT: Z3 evaluate SUCCESS !");
//                state.assignSeed = currentAssign;
//                state.samplingSeedList.push_back(currentAssign);
//                checkValid = true;
//                return ;
//            }
//        }
...
//
//        auto start = std::chrono::high_resolution_clock::now();
//        bool success = invokeQSampler(smtLibStr.c_str(),varTypes, gosatObjects,models,samplingAlg);
//        auto end = std::chrono::high_resolution_clock::now();
//        std::chrono::duration<double> duration = end - start;
//        double milliseconds = duration.count() * 1000.0;
//        llvm::errs() << ">>>optsat exec time: " << milliseconds << " ms\n";
...
//        if (success){
////      llvm::errs()<<"=================GOSAT: GOSAT Result: SAT=================\n";
////            Assignment gosatAssign = Assignment(gosatObjects,models[0],true);
////            Assignment currentAssign = state.assignSeed;
////            currentAssign.updateValues(gosatAssign);
////            int res = checkAssignmentValid(currentAssign,constraints);//检查gosat求出来的解是否 是有效解
////            if (res == 0){
////                klee_warning("Z3QSampler: QSampler solving SAT and evaluate SUCCESS !");
////                state.assignSeed = currentAssign;
//////                state.samplingSeedList.push_back(currentAssign);
////                checkValid = true;
////                return ;
////            }
////            klee_warning("Z3QSampler: QSampler solving SAT and evaluate FAILURE and remove the state !");
//            std::vector<Assignment> tempSeedList;
//            bool flag = false;
////            llvm::errs()<<"models_size: "<<models.size()<<"\n";
//            for(int i=0; i<models.size(); i++) {
//                Assignment gosatAssign = Assignment(gosatObjects,models[i],true);
//                Assignment currentAssign = state.assignSeed;
//                currentAssign.updateValues(gosatAssign);
//                int res = checkAssignmentValid(currentAssign,constraints);//检查gosat求出来的解是否 是有效解
//                if (res == 0){
//                    state.assignSeed = currentAssign;
//                    tempSeedList.push_back(currentAssign);
//                    flag = true;
//                }
//            }
//            if(flag){
//                state.samplingSeedList = tempSeedList;
//                klee_warning(" Z3QSampler: QSampler solving SAT and evaluate SUCCESS !");
//                checkValid = true;
//                return ;
//            }
...
//        }
//        else
//            klee_warning("Z3QSampler: OSampler solving UNKNOWN and evaluate FAILURE and remove the state !");
//    }
//
//    if (! state.fakeState){
//        removedStates.push_back(&state);
...
//
//    auto start = std::chrono::high_resolution_clock::now();
//    bool success = invokeGoSAT(smtLibStr.c_str(),varTypes, objects,values, "gosat");
//    auto end = std::chrono::high_resolution_clock::now();
//    std::chrono::duration<double> duration = end - start;
//    double milliseconds = duration.count() * 1000.0;
//    llvm::errs() << ">>>GoSat exec time: " << milliseconds << " ms\n";
...
void Executor::getConcreteAssignSeedOptSAT(ExecutionState &state, bool &checkValid){
//  llvm::errs()<<"===========call gosat solver===========\n";
    std::vector< std::vector<unsigned char> > values,zero;
    std::vector< const Array*> objects;
    std::map<std::string,std::string> varTypes;
    std::set<std::string> varName;
    ConstraintSet constraints(state.constraints);
...
//    llvm::errs()<<smtLibStr<<"\n";
    ...
    auto start = std::chrono::high_resolution_clock::now();
    bool success = invokeGoSAT(smtLibStr.c_str(),varTypes, objects,values,zero, "optsat");
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> duration = end - start;
    double milliseconds = duration.count() * 1000.0;
    llvm::errs() << ">>>OptSat exec time: " << milliseconds << " ms\n";
...
//      currentAssign.dump();
        int res = checkAssignmentValid(currentAssign,constraints);//检查gosat求出来的解是否 是有效解
        if (res == 0){
            klee_warning("OPTSAT: OPTSAT solving SAT and evaluate SUCCESS !");
            state.assignSeed = currentAssign;
            Assignment assignZero = Assignment(objects,zero,true);
//        assignZero.dump();
            state.assignzeros = assignZero;
            checkValid = true;
            return ;
        }
        klee_warning("OPTSAT: OPTSAT solving SAT and evaluate FAILURE and remove the state !");
    }
    else
        klee_warning("OPTSAT: OPTSAT solving UNKNOWN and evaluate FAILURE and remove the state !");
...
void Executor::getConcreteAssignSeedBitwuzla(ExecutionState &state, bool &checkValid, std::string filename){//no fixed at time
...
//  llvm::outs()<<"===========call cvc5 solver===========\n";
  ...
  ConstraintSet constraints(state.constraints);
    ...
    auto start = std::chrono::high_resolution_clock::now();
    int success = cvc5Solver.invokeCVC5Solver(constraints,&objects, &values);
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> duration = end - start;
    double milliseconds = duration.count() * 1000.0;
    llvm::errs() << ">>>CVC5 exec time: " << milliseconds << " ms\n";

  if (success==1) {
    Assignment cvc4Assign = Assignment(objects,values,true);
    int checki = checkAssignmentValid(cvc4Assign,constraints);
    if (checki == 0){
    ...
    klee_warning("CVC5: CVC5 solving SAT and evaluate FAILURE and remove the state !");
  }
  else if(success==0){
      klee_warning("CVC5: CVC5 solving UNSAT and evaluate FAILURE and remove the state !");
  }
  else{
      klee_warning("CVC5: CVC5 solving UNKNOWN and evaluate FAILURE and remove the state !");
  }
  if (!state.fakeState){
    removedStates.push_back(&state);
    concreteHalt = true;
...
void Executor::getStateSeed(ExecutionState &state, bool &checkValid, std::string filename) {
  if (SolverTypeDecision == SolverType::Z3)//Bit-blasting //   BVFP
//    SMT：约束都是bvfp格式表示，丢给smt solver之前已经转成了bvfp，smt内部直接对bvfp进行操作。
    getConcreteAssignSeedZ3(state,checkValid);
  else if (SolverTypeDecision == SolverType::BITWUZLA)//propagate-based local search bit-blasting
    getConcreteAssignSeedBitwuzla(state,checkValid, filename);
  ...
  else if (SolverTypeDecision == SolverType::DREAL)// only dreal, and with search by jfs
    ...
    getConcreteAssignSeedJFS(state,checkValid,1);
  else if (SolverTypeDecision == SolverType::Z3_GOSAT)//Mathematical optimisation   //search
    getConcreteAssignSeedZ3GoSAT(state,checkValid);
  else if (SolverTypeDecision == SolverType::OPTSAT)//Mathematical optimisation   //search
    getConcreteAssignSeedOptSAT(state,checkValid);
  else if (SolverTypeDecision == SolverType::Z3_DREAL_JFS)//synergy
    getConcreteAssignSeedZ3DRealJFS(state,checkValid, filename);
  else if (SolverTypeDecision == SolverType::Z3_JFS)//synergy
    getConcreteAssignSeedZ3JFS(state,checkValid);
  else if (SolverTypeDecision == SolverType::Z3_OPTSAT) //only gosat, and z3 solver int var
    getConcreteAssignSeedZ3OptSAT(state,checkValid);
//  else if (SolverTypeDecision == SolverType::Z3_QSampler) //only gosat, and z3 solver int var
//    getConcreteAssignSeedZ3QSampler(state,checkValid);
...
//    getConcreteAssignSeedDReal(state,checkValid);
//  else if (SolverTypeDecision == SolverType::Z3_DREAL) // z3 solver int var
//    getConcreteAssignSeedZ3DReal(state,checkValid);
  else
...
void Executor::run(ExecutionState &initialState) {//
  bindModuleConstants();
//  llvm::errs()<<"[init state]: "<<*initialState.pc->inst<<"\n";
...
//    llvm::errs()<<"[states loop]: "<<*state.pc->inst<<"\n";//第一条指令，是call 入口看书main
...
Interpreter *Interpreter::create(LLVMContext &ctx, const InterpreterOptions &opts,
                                 InterpreterHandler *ih) {
  if (FPToIntTransfrom != ""){
    SolverTypeDecision.setInitialValue(SolverType::Z3);
